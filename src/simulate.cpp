...
void create_csv(std::string const& filename,
                std::vector<size_t> const& wordlengths) {
  ...
  for (auto const distance : distances) {
    ...
    std::transform(
        indices.begin(), indices.end(), row.begin(), [&](auto const i) {
          auto sequences = create_sequence_pair(sequence_length, distance);
          auto distance_matrix =
              spam::distance_matrix{std::move(sequences), pattern, wordlengths};
...
int main(int argc, char* argv[]) { create_csv("out.csv", {11, 14}); }
