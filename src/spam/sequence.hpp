...
struct assembled_sequence {
  ...
  auto size() const -> size_t { return nucleotides.size(); }

  auto begin() const { return nucleotides.begin(); }

  auto end() const { return nucleotides.end(); }
};
...
struct sequence
  ...
  auto adjusted_size(size_t wordlength) const -> size_t;

  auto error_rate() const -> long double;
};

auto background_match_probability(spam::sequence const& sequence1,
                                  spam::sequence const& sequence2) -> double;
